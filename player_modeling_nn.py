...
def forwardprop(X, w1, w2):
    ...
    """
    h    = tf.nn.sigmoid(tf.matmul(X, w1))  # The \sigma function
    yhat = tf.matmul(h, w2)  # The \varphi function
    return yhat
...
def get_simple_agent_data():
    ...
    m,n,o,p = train_test_split(all_X, all_Y, test_size=0.25, random_state=RANDOM_SEED)
...
def main():
    ...
    w1 = init_weights((x_size, h_size), 1)
    w2 = init_weights((h_size, y_size), 2)
