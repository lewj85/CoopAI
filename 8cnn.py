...
board_size = 8
filename = '8cnn.json'
loadpath = "cnn8_f32k5_f32k3_lr001.99-0.02.hdf5"
filepath = "cnn8_f32k5_f32k3_lr0001.{epoch:02d}-{val_loss:.2f}.hdf5"
...
def make_cnn():
    ...
    # input
    x = Input(shape=(board_size, board_size, 4,))
    ...
    # 1st hidden layer is 2 cnns
    fivebyfive = Conv2D(32, 5, strides=1, input_shape=(board_size, board_size, 4), kernel_initializer='random_uniform')(x)
    threebythree = Conv2D(32, 3, strides=1, input_shape=(board_size, board_size, 4), kernel_initializer='random_uniform')(x)
    both = Concatenate()([fivebyfive, threebythree], axis=-1)
    model.add(both)
